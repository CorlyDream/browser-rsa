...
function pkcs1pad(s, n, isTypeOne = false) {
    ...
    let i = 0;
    while (i < randomLen) {
        let tmp = utils.random.nextByte();
        if (tmp != 0) { // must nonzero
            randomArr[i++] = tmp;
        }
    }
...
function oaeppad(buffer, k) {
    ...
    const DB = [...lHash,...PS, ...buffer];
    const seed = new Array(hLen);
    utils.random.nextBytes(seed);
...
function oaepunpad(buffer) {
    ...
    if (utils.hex.fromByteArray(lHashEM) != utils.hex.fromByteArray(lHash)) {
